...
public class Driver {
	...
	public void doSQL(int threadId, int sqlId, int paraNumber, Object[] para) {
		int tenantId = ran.nextInt(MysqlWarmUp.tenantPerThread) + threadId * MysqlWarmUp.tenantPerThread;
		doSQLOnce(threadId, tenantId, sqlId, paraNumber, para);
	}

	public boolean doSQLOnce(int threadId, int tenantId, int sqlId, int paraNumber, Object[] para) {
		...
		try{
			...
			case 0: //customer
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM customer"+tenantId+" WHERE c_id = "+para[0]+" AND c_w_id = "+para[1]+" AND c_d_id = "+para[2]);
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE customer"+tenantId+" SET c_id = "+para[0]+", c_d_id = "+para[1]+",c_w_id = "+para[2]+", c_first = '"+para[3]+"', c_middle = '"+para[4]+"', c_last = '"+para[5]+"', c_street_1 = '"+para[6]+"', c_street_2 = '"+para[7]+"',c_city = '"+para[8]+"',"
				...
				+ "WHERE c_id = "+para[21]+" AND c_w_id = "+para[22]+" AND c_d_id = "+para[23]);
					...
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM customer"+tenantId+" WHERE c_id = "+para[0]+" AND c_w_id = "+para[1]+" AND c_d_id = "+para[2]);
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO customer"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+",'"+para[3]+"','"+para[4]+"','"+para[5]+"','"+para[6]+"','"+para[7]+"','"+para[8]+"','"+para[9]+"','"+para[10]+"','"+para[11]+"','"+para[12]+"','"+para[13]+"',"+para[14]+","+para[15]+","+para[16]+","+para[17]+","+para[18]+","+para[19]+",'"+para[20]+"')");
				}
				break;
			case 1:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM district"+tenantId+" WHERE d_w_id = "+para[0]+" AND d_id = "+para[1]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE district"+tenantId+" SET d_id = "+para[0]+", d_w_id = "+para[1]+", d_name = '"+para[2]+"', d_street_1 = '"+para[3]+"', d_street_2 = '"+para[4]+"', d_city = '"+para[5]+"', d_state = '"+para[6]+"', d_zip = '"+para[7]+"', d_tax = "+para[8]+", d_ytd = "+para[9]+", d_next_o_id = "+para[10]+" WHERE d_w_id = "+para[11]+" AND d_id = "+para[12]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM district"+tenantId+" WHERE d_w_id = "+para[0]+" AND d_id = "+para[1]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO district"+tenantId+" VALUES ("+para[0]+","+para[1]+",'"+para[2]+"','"+para[3]+"','"+para[4]+"','"+para[5]+"','"+para[6]+"','"+para[7]+"',"+para[8]+","+para[9]+","+para[10]+")");
				}
				break;
			case 2:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM item"+tenantId+" WHERE i_id = "+para[0]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE item"+tenantId+" SET i_id = "+para[0]+", i_im_id = "+para[1]+", i_name = '"+para[2]+"', i_price = "+para[3]+", i_data = '"+para[4]+"' WHERE i_id = "+para[5]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM item"+tenantId+" WHERE i_id = "+para[0]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO item"+tenantId+" VALUES ("+para[0]+","+para[1]+",'"+para[2]+"',"+para[3]+",'"+para[4]+"')");
				}
				break;
			case 3:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM new_orders"+tenantId+" WHERE no_w_id = "+para[0]+" AND no_d_id = "+para[1]+" AND no_o_id = "+para[2]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE new_orders"+tenantId+" SET no_o_id = "+para[0]+",no_d_id = "+para[1]+",no_w_id = "+para[2]+" WHERE no_w_id = "+para[3]+" AND no_d_id = "+para[4]+" AND no_o_id = "+para[5]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM new_orders"+tenantId+" WHERE no_w_id = "+para[0]+" AND no_d_id = "+para[1]+" AND no_o_id = "+para[2]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO new_orders"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+")");
				}
				break;
			case 4:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM order_line"+tenantId+" WHERE ol_w_id = "+para[0]+" AND ol_d_id = "+para[1]+" AND ol_o_id = "+para[2]+" AND ol_number = "+para[3]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE order_line"+tenantId+" SET ol_o_id = "+para[0]+", ol_d_id = "+para[1]+",ol_w_id = "+para[2]+",ol_number = "+para[3]+",ol_i_id = "+para[4]+", ol_supply_w_id = "+para[5]+",ol_delivery_d = '"+para[6]+"', ol_quantity = "+para[7]+", ol_amount = "+para[8]+", ol_dist_info = '"+para[9]+"' WHERE ol_w_id = "+para[10]+" AND ol_d_id = "+para[11]+" AND ol_o_id = "+para[12]+" AND ol_number = "+para[13]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM order_line"+tenantId+" WHERE ol_w_id = "+para[0]+" AND ol_d_id = "+para[1]+" AND ol_o_id = "+para[2]+" AND ol_number = "+para[3]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO order_line"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+","+para[3]+","+para[4]+","+para[5]+",'"+para[6]+"',"+para[7]+","+para[8]+",'"+para[9]+"')");
				}
				break;
			case 5:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM orders"+tenantId+" WHERE o_w_id = "+para[0]+" AND o_d_id = "+para[1]+" AND o_id = "+para[2]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE orders"+tenantId+" SET o_id = "+para[0]+", o_d_id = "+para[1]+", o_w_id = "+para[2]+",o_c_id = "+para[3]+",o_entry_d = '"+para[4]+"',o_carrier_id = "+para[5]+",o_ol_cnt = "+para[6]+", o_all_local = "+para[7]+" WHERE o_w_id = "+para[8]+" AND o_d_id = "+para[9]+" AND o_id = "+para[10]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM orders"+tenantId+" WHERE o_w_id = "+para[0]+" AND o_d_id = "+para[1]+" AND o_id = "+para[2]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO orders"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+","+para[3]+",'"+para[4]+"',"+para[5]+","+para[6]+","+para[7]+")");
				}
				break;
			case 6:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM stock"+tenantId+" WHERE s_w_id = "+para[0]+" AND s_i_id = "+para[1]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE stock"+tenantId+" SET s_i_id = "+para[0]+", s_w_id = "+para[1]+", s_quantity = "+para[2]+", s_dist_01 = '"+para[3]+"', s_dist_02 = '"+para[4]+"',s_dist_03 = '"+para[5]+"',s_dist_04 = '"+para[6]+"', s_dist_05 = '"+para[7]+"', s_dist_06 = '"+para[8]+"', s_dist_07 = '"+para[9]+"', s_dist_08 = '"+para[10]+"', s_dist_09 = '"+para[11]+"', s_dist_10 = '"+para[12]+"', s_ytd = "+para[13]+", s_order_cnt = "+para[14]+", s_remote_cnt = "+para[15]+",s_data = '"+para[16]+"' WHERE s_w_id = "+para[17]+" AND s_i_id = "+para[18]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM stock"+tenantId+" WHERE s_w_id = "+para[0]+" AND s_i_id = "+para[1]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO stock"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+",'"+para[3]+"','"+para[4]+"','"+para[5]+"','"+para[6]+"','"+para[7]+"','"+para[8]+"','"+para[9]+"','"+para[10]+"','"+para[11]+"','"+para[12]+"',"+para[13]+","+para[14]+","+para[15]+",'"+para[16]+"')");
				}
				break;
			case 7:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM warehouse"+tenantId+" WHERE w_id = "+para[0]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE warehouse"+tenantId+" SET w_id = "+para[0]+",	w_name = '"+para[1]+"',w_street_1 = '"+para[2]+"',w_street_2 = '"+para[3]+"',w_city = '"+para[4]+"',w_state = '"+para[5]+"',w_zip = '"+para[6]+"',w_tax = "+para[7]+",	w_ytd = "+para[8]+" WHERE w_id = "+para[9]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM warehouse"+tenantId+" WHERE w_id = "+para[0]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO warehouse"+tenantId+" VALUES ("+para[0]+",'"+para[1]+"','"+para[2]+"','"+para[3]+"','"+para[4]+"','"+para[5]+"','"+para[6]+"',"+para[7]+","+para[8]+")");
				}
				break;
			case 8:
				if(queryId == 0){//select
					MysqlWarmUp.warmupThread[threadId].stmt.execute("SELECT * FROM history"+tenantId+" WHERE h_c_id = "+para[0]+" AND h_c_d_id = "+para[1]+" AND h_c_w_id = "+para[2]+"");
				}else if(queryId == 1){//update
					MysqlWarmUp.warmupThread[threadId].stmt.execute("UPDATE history"+tenantId+" SET h_c_id = "+para[0]+", h_c_d_id = "+para[1]+", h_c_w_id = "+para[2]+",h_d_id = "+para[3]+",h_w_id = "+para[4]+",h_date = '"+para[5]+"',h_amount = "+para[6]+",h_data = '"+para[7]+"' WHERE h_c_id = "+para[8]+" AND h_c_d_id = "+para[9]+" AND h_c_w_id = "+para[10]+"");
				}else if(queryId == 2){//delete
					MysqlWarmUp.warmupThread[threadId].stmt.execute("DELETE FROM history"+tenantId+" WHERE h_c_id = "+para[0]+" AND h_c_d_id = "+para[1]+" AND h_c_w_id = "+para[2]+"");
				}else if(queryId == 3){//insert
					MysqlWarmUp.warmupThread[threadId].stmt.execute("INSERT INTO history"+tenantId+" VALUES ("+para[0]+","+para[1]+","+para[2]+","+para[3]+","+para[4]+",'"+para[5]+"',"+para[6]+",'"+para[7]+"')");
				}
				break;
			}
			return success;
