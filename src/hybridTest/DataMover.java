...
public class DataMover {
	...
	public void Mysql2Voltdb(int tenantId) throws SQLException, NoConnectionsException, IOException, ProcCallException{
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectWarehouse", tenantId, rs.getInt("w_id"));
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectDistrict", tenantId, rs.getInt("d_id"), rs.getInt("d_w_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectCustomer", tenantId, rs.getInt("c_w_id"), rs.getInt("c_d_id"), rs.getInt("c_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}
		...
		while(rs.next()){ //there are no updates to history
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectNewOrders", tenantId, rs.getInt("no_o_id"), rs.getInt("no_d_id"), rs.getInt("no_w_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}			
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("PRocedureSelectOrders", tenantId, rs.getInt("o_id"), rs.getInt("o_d_id"), rs.getInt("o_w_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}		
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectOrderLine", tenantId, rs.getInt("ol_o_id"), rs.getInt("ol_d_id"), rs.getInt("ol_w_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}		
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectItem", tenantId, rs.getInt("i_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}		
		...
		while(rs.next()){
			response = voltdbConn.callProcedure("ProcedureSelectStock", tenantId, rs.getInt("s_i_id"), rs.getInt("s_w_id"));
			if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
				continue;
			}		
	...
	public void Voltdb2Mysql(int tenantId){
		...
		dr.start();
		HistoryRetriver hr = new HistoryRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		hr.start();
		NewOrdersRetriver nor = new NewOrdersRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		nor.start();
		OrderLineRetriver olr = new OrderLineRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		olr.start();
		OrdersRetriver or = new OrdersRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		or.start();
		StockRetriver sr = new StockRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		sr.start();
		WarehouseRetriver wr = new WarehouseRetriver(this.dbURL, this.dbUsername, this.dbPassword, this.voltdbServer, tenantId, 1);
		wr.start();
		try {
			cr.join();
			dr.join();
