...
public class ItemRetriver extends Thread {
	...
	public void run(){
		...
		VoltTable result = null;
			try{
				response = voltdbConn.callProcedure("@AdHoc", "SELECT * FROM item"+volumnId+" WHERE tenant_id = "+tenantId+" AND is_insert = 0 AND is_update = 1");
				...
				response = voltdbConn.callProcedure("@AdHoc", "SELECT * FROM item"+volumnId+" WHERE tenant_id = "+tenantId+" AND is_insert = 1");
				if(response.getStatus() == ClientResponse.SUCCESS && response.getResults()[0].getRowCount() != 0){
					result = response.getResults()[0];
					for(int i = 0; i<result.getRowCount(); i++){
						VoltTableRow row = result.fetchRow(i);
