	...
	public boolean doSQLOnce(int threadId, int sqlId, int paraNumber, Object[] para,
		...
		try {
			for (i = 0; i < paraNumber; i++) {
				switch (paraType[i]) {
				case 0:
					Tenant.statements[threadId][sqlId].setInt(i+1, (int) para[i]);
					break;
				case 1:
					Tenant.statements[threadId][sqlId].setString(i+1, (String)para[i]);
					break;
				case 2:
					Tenant.statements[threadId][sqlId].setFloat(i+1, (float)para[i]);
					break;
				case 3:
					Tenant.statements[threadId][sqlId].setDouble(i+1, (double)para[i]);
					break;
					default:
				}
			}
